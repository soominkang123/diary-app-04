...
const mockData = [
 ...
 /* 상태 관리 : context , 리덕스 , 몹엑스 .... 
     ...
     1. const DiaryStateConxext = React.createContext() 를 사용해서 context 생성
     2. provider 를 사용해서 컴퍼넌트를 묶어 줘야함. , state를 내려 보내 줄수 있다. 
     
     3. useContext : 상태 값을 가져와서 사용함.
     4. useReducer 를 사용해서 context의 상태 값을 변경함.  
 */

 //  1. Context 선언 : 상태 값을 처리 , 이벤트 처리
   export const DiaryStateContext = React.createContext();   // 상태 값을 전송 하는 context
 ...
 // 상태 값을 변경하는 reducer 함수 정의 
   function reducer ( state , action) {
     switch (action.type) {
      ...
      case "DELETE":
        // state.filter 을 돌려서 action.targetId !== it.id 를 새로운 배열에 담아서 리턴
...
function App() {
  ...
  // 컴포넌트가 처음 로드될때 dispatch 를 호출해서 data에 mockDate 의 값을 할당.
  useEffect ( () => {
    dispatch ({
        type:"INIT" ,
        data:mockData,
